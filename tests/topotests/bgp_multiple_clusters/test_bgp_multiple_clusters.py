...
They each supersead two clusters of clients
"""
...
def check_routes_cluster_list(router, prefixs, expected_cluster_lists):
    """Check that router receives a prefix with the correct cluster list"""

    if len(prefixs)!=len(expected_cluster_lists):
    ...
    if(totalRoutes!=len(prefixs)+1): #we don't Check for the loopback address
        logger.info(
            "{}: expected {} prefixs, got {} (waiting)".format(
                router.name,totalRoutes,len(prefixs)+1
            )
        )
        return None
    ...
    for i,prefix in enumerate(prefixs):
        try:
            ...
            if len(paths) > 1:
                logger.info(
                    "{}: Expected 1 paths, got {} (waiting)".format(
...
def test_bgp_multiple_cluster_cluster_lists():
    ...
    logger.info("Testing cluster-lists of prefixs on r1, r2, r3, r6, r7, r8 and r9")
...
def test_client_to_client_reflection_per_neighbor_cluster():
    ...
    logger.info("Testing cluster-lists of prefixs on r1, r2, r3 and r9 with no reflection in cluster 20.0.0.1 ")

    r4.vtysh_cmd("configure \n\
                  router bgp 100 \n\
                  bgp cluster-id per-neighbor 20.0.0.1 client-to-client-reflection false\n")
    ...
    logger.info("Testing cluster-lists of prefixs on r1, r2, r3 and r9 with no reflection in cluster 20.0.0.1 and no client-to-client reflection")
    ...
    logger.info("Testing cluster-lists of prefixs on r1, r2, r3 and r9 with reflection in cluster 20.0.0.1 and no client-to-client reflection")

    r4.vtysh_cmd("""
                 configure 
                 router bgp 100 
                 bgp cluster-id per-neighbor 20.0.0.1 client-to-client-reflection true
                 """)
...
def test_client_to_client_reflection_global_cluster():
    ...
    logger.info("Testing cluster-lists of prefixs on r1, r2, r3 and r9 with no reflection in global cluster")
    r4.vtysh_cmd("""
                  configure
                  router bgp 100 
                  bgp cluster-id global client-to-client-reflection false
    ...
    logger.info("Testing cluster-lists of prefixs on r1, r2, r3 and r9 with no reflection in global cluster and no client-to-client reflection")
    ...
    logger.info("Testing cluster-lists of prefixs on r1, r2, r3 and r9 with reflection in global cluster and no client-to-client reflection")

    r4.vtysh_cmd("configure \n\
                  router bgp 100 \n\
                  bgp cluster-id global client-to-client-reflection true\n")
    ...
    r4.vtysh_cmd("configure \n\
                  ...
                  no bgp cluster-id global 20.0.0.1" )
