		      ...
		      safi_t safi)
{
	...
	dst->per_neighbor_cluster[afi][safi] = src->per_neighbor_cluster[afi][safi];
...
#define SEED3 4258594758
	...
	if (CHECK_FLAG(peer->flags, PEER_FLAG_AS_LOOP_DETECTION))
		key = jhash_2words(peer->as,
				   ...
				   key);
	if (CHECK_FLAG(flags, PEER_FLAG_PER_NEIGHBOR_CLUSTER_ID))
		key = jhash_1word(peer->per_neighbor_cluster[afi][safi].s_addr, key);
	if (peer->group)
...
static bool updgrp_hash_cmp(const void *p1, const void *p2)
{
	...
	if (CHECK_FLAG(flags1, PEER_FLAG_PER_NEIGHBOR_CLUSTER_ID) &&
	    CHECK_FLAG(flags2, PEER_FLAG_PER_NEIGHBOR_CLUSTER_ID) &&
	    !IPV4_ADDR_SAME(&pe1->per_neighbor_cluster[afi][safi],
			    &pe2->per_neighbor_cluster[afi][safi]))
		return false;
