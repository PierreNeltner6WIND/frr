	...
	if (newtype != BGP_PEER_IBGP) {
		...
		FOREACH_AFI_SAFI (afi, safi) {
			if (CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_CLUSTER_ID)) {
				...
				peer->cluster[afi][safi].s_addr = 0;
				peer->cluster_flags[afi][safi] = 0;
					   ...
					   safi_t safi)
{
	...
	if (!CHECK_FLAG(pflags_ovrd, PEER_FLAG_CLUSTER_ID)) {
		...
		PEER_ATTR_INHERIT(peer, group, cluster_flags[afi][safi]);
		if (CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_CLUSTER_ID))
...
uint8_t copy_global_cluster_flags(struct bgp *bgp)
{
	uint8_t flags = 0;

	if (CHECK_FLAG(bgp->flags, BGP_FLAG_CLIENT_TO_CLIENT_GLOBAL_CLUSTER_CONFIGURED))
		SET_FLAG(flags, CLUSTER_FLAG_CLIENT_TO_CLIENT_INTRA_CLUSTER_CONFIGURED);
	if (CHECK_FLAG(bgp->flags, BGP_FLAG_CLIENT_TO_CLIENT_GLOBAL_CLUSTER))
		SET_FLAG(flags, CLUSTER_FLAG_CLIENT_TO_CLIENT_INTRA_CLUSTER);
	SET_FLAG(flags, CLUSTER_FLAG_GLOBAL);
	return flags;
}
		...
		FOREACH_AFI_SAFI (afi, safi) {
			...
			if (CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_CLUSTER_ID)) {
				...
				if (!CHECK_FLAG(cluster->flags, CLUSTER_FLAG_GLOBAL))
					peer->cluster_flags[afi][safi] = cluster->flags;
				else
					peer->cluster_flags[afi][safi] =
						copy_global_cluster_flags(bgp);
			}
		}
	}
}
				 ...
				 afi_t afi, safi_t safi)
{
	...
	if (CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_CLUSTER_ID) &&
	    IPV4_ADDR_SAME(&peer->cluster[afi][safi], cluster_id))
		return;
	...
	if (!CHECK_FLAG(cluster->flags, CLUSTER_FLAG_GLOBAL))
		...
		peer->cluster_flags[afi][safi] = copy_global_cluster_flags(bgp);
	...
	for (ALL_LIST_ELEMENTS(peer->group->peer, node, nnode, member)) {
		...
		member->cluster_flags[afi][safi] = peer->cluster_flags[afi][safi];
...
void bgp_neighbor_cluster_id_unset(struct bgp *bgp, struct peer *peer, afi_t afi, safi_t safi)
{
	...
	if (peer_group_active(peer)) {
		...
		PEER_ATTR_INHERIT(peer, peer->group, cluster_flags[afi][safi]);
	...
	for (ALL_LIST_ELEMENTS(peer->group->peer, node, nnode, member)) {
		 ...
		 * there flags are already down thanks to
		 * peer_af_flag_unset
		 */
		member->cluster[afi][safi].s_addr = 0;
		member->cluster_flags[afi][safi] = peer->cluster_flags[afi][safi];
