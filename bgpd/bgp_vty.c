	...
	"Route-Reflector Cluster-id as 32 bit quantity\n")
{
	...
	if (no)
		bgp_neighbor_cluster_id_unset(bgp, &cluster, peer, afi, safi);
	else
			      ...
			      json_object *json_neigh)
{
	...
	if (use_json) {
		...
		if (CHECK_FLAG(p->af_flags[afi][safi], PEER_FLAG_PER_NEIGHBOR_CLUSTER_ID))
				     ...
				     struct peer *peer, afi_t afi, safi_t safi)
{
	...
	if (peergroup_af_flag_check(peer, afi, safi, PEER_FLAG_PER_NEIGHBOR_CLUSTER_ID)) {
		cluster_id = peer->per_neighbor_cluster[afi][safi];
		...
		for (ALL_LIST_ELEMENTS(bgp->per_neighbor_clusters, node, nnode, cluster)) {
			if (CHECK_FLAG(cluster->flags,
				...
				if (CHECK_FLAG(cluster->flags,
					       CLUSTER_FLAG_CLIENT_TO_CLIENT_INTRA_CLUSTER))
					vty_out(vty,
						" bgp cluster-id per-neighbor %pI4 client-to-client-reflection true\n",
						&cluster->cluster_id);
				else {
					vty_out(vty,
						" bgp cluster-id per-neighbor %pI4 client-to-client-reflection false\n",
						&cluster->cluster_id);
				}
			}
		}
